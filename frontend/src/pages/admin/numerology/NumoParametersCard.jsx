...
import { Card, Tag, Rate, Typography, Tooltip } from 'antd';
import { InfoCircleOutlined, FireOutlined, HeartOutlined, TeamOutlined, CompassOutlined, StarOutlined, EyeOutlined } from '@ant-design/icons';

const { Text, Paragraph } = Typography;

const ICONS = {
  stillness: <EyeOutlined style={{ color: '#6366f1' }} />,
  luck: <FireOutlined style={{ color: '#f59e0b' }} />,
  harmony: <TeamOutlined style={{ color: '#06b6d4' }} />,
  destiny: <CompassOutlined style={{ color: '#8b5cf6' }} />,
  karmic: <HeartOutlined style={{ color: '#ec4899' }} />,
  intuition: <StarOutlined style={{ color: '#eab308' }} />,
};
        ...
        {params.map(p => (
          <div key={p.key} style={{ border: `1px solid ${p.tone === 'good' ? 'rgba(79,143,125,0.28)' : p.tone === 'watch' ? 'rgba(185,125,104,0.28)' : 'rgba(199,154,75,0.18)'}`, background: '#1B1D25', borderRadius: 12, padding: '12px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              <Rate disabled value={p.score} count={5} style={{ fontSize: 14 }} />
