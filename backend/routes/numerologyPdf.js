...
function safeY(doc, needed) {
  let y = doc.y;
  if (y + needed > PAGE_H - 60) { doc.addPage(); y = MARGIN; }
  return y;
}
...
router.get('/employees/:id/numerology/pdf', authPdf, (req, res) => {
  try {
    ...
    if (core.lifePath || core.expression) {
      y = sectionTitle(doc, 'CORE NUMBERS');
      const nums = [
    ...
    if (triNature.signature) {
      y = sectionTitle(doc, 'SIGNATURE');
      const sig = triNature.signature;
      doc.save();
      doc.roundedRect(MARGIN, y, CONTENT_W, 55, 8).fill('#f2f5ff');
      doc.fontSize(14).fillColor('#3d5df0').font('Helvetica-Bold').text(sig.name, MARGIN + 12, y + 10, { width: CONTENT_W - 24 });
      doc.fontSize(9).fillColor('#5c6580').font('Helvetica').text(sig.desc, MARGIN + 12, y + 30, { width: CONTENT_W - 24 });
      doc.restore();
      y += 65;
    }
    ...
    if (triNature.categories) {
      y = sectionTitle(doc, '6 CATEGORIES');
      ...
      cats.forEach(([key, cat], i) => {
        ...
        const cy = y + row * (catH + 8);
        if (cy + catH > PAGE_H - 60) { doc.addPage(); }
        ...
        if (cat.score != null) {
          ...
          scoreBar(doc, cx + 10, cy + 26, halfW - 20, 8, cat.score, CAT_COLORS[key] || '#3d5df0');
        } else {
      ...
      y += Math.ceil(cats.length / 2) * (catH + 8) + 10;
    }

    if (triNature.maskedTraits && triNature.maskedTraits.length) {
      y = sectionTitle(doc, 'MASKED TRAITS — INTERVIEW SCENARIOS');
    ...
    if (triNature.elements) {
      y = sectionTitle(doc, 'ELEMENT SCORES');
      Object.entries(triNature.elements).sort((a, b) => b[1] - a[1]).forEach(([el, score]) => {
        if (y + 20 > PAGE_H - 60) { doc.addPage(); y = MARGIN; }
        doc.save();
        ...
        scoreBar(doc, MARGIN + 175, y + 2, 280, 10, score, ELEMENT_COLORS[el] || '#3d5df0');
        doc.restore();
        y += 22;
      });
      y += 10;
    ...
    if (triNature.parameters) {
      y = sectionTitle(doc, 'BEHAVIORAL PARAMETERS');
      Object.entries(triNature.parameters).sort((a, b) => b[1].score - a[1].score).forEach(([pname, p]) => {
        ...
        scoreBar(doc, MARGIN + 190, y + 1, 260, 7, p.score, ELEMENT_COLORS[p.element] || '#3d5df0');
        doc.restore();
        y += 14;
      });
      y += 10;
    ...
    if (numoParams.length) {
      y = sectionTitle(doc, 'NUMEROLOGY LENSES');
      numoParams.forEach(p => {
        ...
        scoreBar(doc, MARGIN + 360, y + 1, 90, 7, p.score * 20, '#C79A4B');
        doc.restore();
        y += 14;
      });
      y += 10;
    ...
    if (conclusion) {
      y = sectionTitle(doc, 'OVERALL CONCLUSION');
      ...
      if (conclusion.greenFlags && conclusion.greenFlags.length) {
        ...
        conclusion.greenFlags.forEach(g => {
          if (y + 14 > PAGE_H - 60) { doc.addPage(); y = MARGIN; }
      ...
      if (conclusion.finalVerdict) {
        if (y + 70 > PAGE_H - 60) { doc.addPage(); y = MARGIN; }
        doc.save();
